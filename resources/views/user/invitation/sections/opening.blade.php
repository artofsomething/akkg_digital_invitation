<!-- resources/views/user/invitation/sections/opening.blade.php -->
<form onsubmit="saveSection(event, '{{ $invitation->slug }}', '{{ $section->id }}')">
    <div class="space-y-4">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Opening Title</label>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Music URL (optional)</label>
            <input type="url" name="content[music_url]"
                   value="{{ $section->content['music_url'] ?? '' }}"
                   placeholder="https://..."
                   class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">
        </div>
        <button type="submit"
